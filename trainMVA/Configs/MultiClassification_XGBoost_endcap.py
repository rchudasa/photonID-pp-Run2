...
####Start here
#####################################################################
OutputDirName = 'MultiClassification_XGBoost_endcap' #All plots, models, config file will be stored here
branches=['scE',
          'scEt',
          'scEta',
          'scPhi',
...
processes = [
    ...
    'treeName':'genMatchedBMMGSCTree', 'selection':'(scEt>4) & (scEt<15) & abs(scEta)>1.566 & abs(scEta)<2.4', 'process':'bsMMG', 'category':0},
    {'Class':'QCD','fileName':'mc_qcd20To30EmEnriched_endcap.root',
    'treeName':'mergedPi0_SCTree', 'selection':'(scEt>4) & (scEt<15) & abs(scEta)>1.566 & abs(scEta) < 2.4', 'process':'QCD','category':1}, 
    {'Class':'Pi0','fileName':'mc_bkg_flat_pi0_endcap.root',
    'treeName':'mergedPi0_SCTree', 'selection':'(scEt>4) & (scEt<15) & abs(scEta)>1.566 & abs(scEta) < 2.4', 'process':'Flat_pi0','category':2}, 
    {'Class':'Data','fileName':'data_2018D_endcap.root',
    'treeName':'dataAllSCTree', 'selection':'(scEt>4) & (scEt<15) & abs(scEta)>1.566 & abs(scEta) < 2.4', 'process':'Data','category':3}     
]
